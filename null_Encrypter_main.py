...
class NullEncrypterApp(App):
    ...
    def build(self):
        ...
        # Setup log file in Downloads folder
        try:
        ...
        # Separator
        separator = Label(size_hint_y=None, height=1, canvas=None)
    ...
    def decrypt_text(self, _instance):
        ...
        self.log("Decrypt Text button pressed")
        input_text = self.input_text_widget.text.strip()
        if input_text:
            try:
                self.log(f"Decrypting text ({len(input_text)} chars)...")
                decrypted_text = encryption_logic.decrypt_text(input_text)
                self.output_text_widget.text = decrypted_text
                self.log("Text decrypted successfully")
            except ValueError as e:
